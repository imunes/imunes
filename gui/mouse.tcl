...
proc button3node { c x y } {
	...
	#
	if { $oper_mode == "exec" && $type ni "ext extnat" } {
...
proc removeIPv4nodes {} {
	...
	foreach iface [ifcList $node_id] {
	    set removed_addrs [concat $removed_addrs [getIfcIPv4addrs $node_id $iface]]
	    setIfcIPv4addrs $node_id $iface ""
	}
    }

    setToRunning "ipv4_used_list" [removeFromList [getFromRunning "ipv4_used_list"] $removed_addrs 1]
...
proc removeIPv6nodes {} {
	...
	foreach iface [ifcList $node_id] {
	    set removed_addrs [concat $removed_addrs [getIfcIPv6addrs $node_id $iface]]
	    setIfcIPv6addrs $node_id $iface ""
	}
    }

    setToRunning "ipv6_used_list" [removeFromList [getFromRunning "ipv6_used_list"] $removed_addrs 1]
