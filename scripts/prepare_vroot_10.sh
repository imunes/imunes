...
export ASSUME_ALWAYS_YES=YES pkg bootstrap
...
#else
#    fetched with http://distcache.FreeBSD.org/ports-distfiles/pkg-1.2.7.tar.xz
#    PKGfn="pkg-1.2.7.tar.xz"
#    cp $PKGfn /usr/ports/distfiles/
...
if [ $offline -eq 0 ] && [ $INCOMPLETE == 1 ]; then
	...
	pkg -c $VROOT_MASTER install -fy $pkg >> $LOG
	if [ $? -ne 0 ]; then
...
for pkg in ${notmissing}; do
    pkg_add -F $WORKDIR/packages/$pkg.txz -C $VROOT_MASTER >> $LOG 
    if [ $? -ne 0 ]; then
	err_list="$pkg $err_list"
    fi
done
